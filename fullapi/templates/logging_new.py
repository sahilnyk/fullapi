"""Logging templates restructured into core/logging/ subdirectory."""

# core/logging/__init__.py
LOGGING_INIT = '''"""Logging package for FastAPI application."""
...
class LoggingConfig:
    ...
    def __init__(self):
        ...
        self.file_log_format: str = os.getenv("FILE_LOG_FORMAT", "%(asctime)s - %(name)s - %(levelname)s - %(module)s - %(funcName)s - %(message)s")
        ...
        self.request_log_headers: List[str] = os.getenv("REQUEST_LOG_HEADERS", "User-Agent,Content-Type,Accept").split(",") if os.getenv("REQUEST_LOG_HEADERS") else []
        self.request_log_exclude_paths: List[str] = os.getenv("REQUEST_LOG_EXCLUDE_PATHS", "/health,/metrics,/static").split(",") if os.getenv("REQUEST_LOG_EXCLUDE_PATHS") else ["/health", "/metrics", "/static"]
        ...
        self.db_log_level: str = os.getenv("DB_LOG_LEVEL", "DEBUG").upper()
...
LOGGING_FORMATTERS = '''"""Custom logging formatters and filters."""
...
from .config import LoggingConfig
from .formatters import ColorFilter, ErrorOnlyFilter
