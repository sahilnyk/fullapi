"""Middleware templates restructured into core/middleware/ subdirectory."""

# core/middleware/__init__.py
MIDDLEWARE_INIT = '''"""Middleware package for FastAPI application."""

from .setup import setup_middleware, get_middleware_config
...
# core/middleware/config.py
MIDDLEWARE_CONFIG = '''"""Middleware configuration settings."""
...
class MiddlewareConfig:
    ...
    def __init__(self):
        ...
        self.request_id_enabled: bool = self._get_bool_env("REQUEST_ID_ENABLED", True)
        self.request_id_header: str = os.getenv("REQUEST_ID_HEADER", "X-Request-ID")
...
MIDDLEWARE_SECURITY_HEADERS = '''"""Security headers middleware."""
...
# core/middleware/request_id.py (NEW)
MIDDLEWARE_REQUEST_ID = '''"""Request ID middleware for request tracing."""
...
class RequestIDMiddleware(BaseHTTPMiddleware):
    ...
    This helps with request tracing, logging, and debugging.
    """
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not self.config.request_id_enabled:
            return await call_next(request)
...
MIDDLEWARE_REQUEST_LOGGING = '''"""Request logging middleware."""
...
def setup_middleware(app: FastAPI, config: Optional[MiddlewareConfig] = None):
    ...
    Last registered = outermost = executes first on incoming request.
    
    Execution order: CORS → RequestID → RequestLogging → RateLimit
                     → SecurityHeaders → GZip → route
    """
    middleware_config = config or MiddlewareConfig()
    
    # 1st registered = innermost, runs last (before route handler)
    ...
    # Add rate limiting middleware (block early before processing)
    ...
    # Add request ID middleware (sets request_id before logging)
    if middleware_config.request_id_enabled:
        app.add_middleware(RequestIDMiddleware, config=middleware_config)
    
    # Last registered = outermost, executes first (handles preflight)
    if middleware_config.cors_origins:
        app.add_middleware(
            CORSMiddleware,
            allow_origins=middleware_config.cors_origins,
            allow_credentials=middleware_config.cors_allow_credentials,
            allow_methods=middleware_config.cors_allow_methods,
            allow_headers=middleware_config.cors_allow_headers,
            expose_headers=middleware_config.cors_expose_headers,
        )
